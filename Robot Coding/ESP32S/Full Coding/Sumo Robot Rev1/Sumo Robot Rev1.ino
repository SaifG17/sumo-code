...
void loop() {
  ...
  // if (inputLineR <= 3500 || inputLineL <= 3500) {  //Check the sensor output
  //   if (inputLineR <= 3500) {
  ...
  // Standby, turn at one place when detect black
  // if (inputLineR >= 3500 || inputLineL >= 3500) {  //Check the sensor output
  //   standby();
  // }
  ...
  if (distanceR <= 30 || distanceL <= 30) {  //Check the sensor output
    attack();
    delay(2000);
    digitalWrite(MotorRPin1, LOW);  // Right Stop
    digitalWrite(MotorRPin2, LOW);
