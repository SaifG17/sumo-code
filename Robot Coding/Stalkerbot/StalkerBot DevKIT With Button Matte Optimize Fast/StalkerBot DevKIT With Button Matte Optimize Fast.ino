...
void loop() {
  ...
  while (buttonState == HIGH) {
    ...
    if (inputLineR <= 2800 || inputLineL <= 3000) {  //Check the sensor output
      if (inputLineR <= 2800) {
...
void stalking() {
  if (distanceR < 10 || distanceL < 10) {
    ...
    digitalWrite(MotorLPin1, LOW);  // Left Forward
    digitalWrite(MotorLPin2, HIGH);
    ledcWrite(pwmChannelR, dutyCycleHigher);
    ledcWrite(pwmChannelL, dutyCycleHigher);
