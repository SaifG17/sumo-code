...
int dutyCycleRHigh = 220;
int dutyCycleRLow = 180;
int dutyCycleLHigh = 220;
int dutyCycleLLow = 180;
...
void loop() {
  ...
  if (inputLineR <= 2500 || inputLineL <= 2800) {  //Check the sensor output
    if (inputLineR <= 2500) {
    ...
    if (distanceR > 25 && distanceL > 25) {  //if both sensors read more than 25 cm, Turn at one place
...
void standby() {
  digitalWrite(MotorRPin1, LOW);  // Right Reverse
  digitalWrite(MotorRPin2, HIGH);
  digitalWrite(MotorLPin1, HIGH);  // Left Forward
  digitalWrite(MotorLPin2, LOW);
  ledcWrite(pwmChannelR, dutyCycleRHigh);
  ledcWrite(pwmChannelL, dutyCycleLHigh);
